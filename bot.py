...
async def status_task():
    while True:
        await client.change_presence(game=discord.Game(name='>riddle | Riddle Games', type=3))
...
async def whois(ctx, user: discord.Member=None):
    if user is None:
      await client.say('Please tag a user to get user information. Example- ``>whois @user``')
    else:
      r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
